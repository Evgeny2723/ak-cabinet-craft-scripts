...
function initApp() {
    var reviewsSwiper = new Swiper(".reviews-swiper", {
        ...
        breakpoints: { 767: { slidesPerView: "auto", centeredSlides: !0, spaceBetween: 16 } },
    });
    var testimonialsSwiper = new Swiper(".testimonials-swiper", {
        ...
        spaceBetween: 8,
        navigation: { nextEl: ".btn-next", prevEl: ".btn-prev" },
        breakpoints: { 767: { slidesPerView: 2, centeredSlides: !1, spaceBetween: 16 } },
    ...
    var advantagesSwiper = new Swiper(".advantages-swiper", {
        ...
        spaceBetween: 8,
        navigation: { nextEl: ".btn-next", prevEl: ".btn-prev" },
        breakpoints: { 767: { slidesPerView: "auto", centeredSlides: !1, spaceBetween: 16 } },
    });
        ...
        mm.add({ isDesktop: "(min-width: 768px)", isMobile: "(max-width: 767px)" }, (context) => {
            ...
            processCards.forEach((card, i) => {
                ...
                tl.fromTo(
                    card,
                    { y: "100vh", scale: 1.1 },
                    { y: `${ladderOffset}vh`, scale: finalScale, ease: "none", duration: 1 },
                    i * 1
                );
            });
            tl.to(
                processCards,
                { y: "-100vh", scale: 0, ease: "power2.in", duration: 1, stagger: 0.1 },
                processCards.length
            );
            return () => {
                tl.kill();
    ...
    if (qualitySection) {
        ...
        wrappers.forEach((wrapper, i) => {
            ...
            gsap.to(images[i], {
                scale: 1,
                ease: "power1.inOut",
                scrollTrigger: { trigger: qualitySection, start: `${start}px top`, end: `${end}px top`, scrub: !0 },
            });
            ScrollTrigger.create({
                ...
                scrub: !0,
                onEnter: () => addActive(i),
                onEnterBack: () => addActive(i),
                onLeave: () => {
                    if (i < count - 1) removeActive(i);
                },
                onLeaveBack: () => {
                    if (i > 0) removeActive(i);
                },
            });
            ScrollTrigger.create({
                trigger: qualitySection,
                ...
                onUpdate: (self) => {
                    lines.forEach((ln, j) => {
                        if (j === i) {
                            ln.style.opacity = 1;
                            ln.style.width = `${self.progress * 100}%`;
                        } else {
                            ln.style.opacity = 0;
                            ln.style.width = "0%";
                        }
    ...
    document.querySelectorAll(".project-item").forEach((item) => {
        ...
        gsap.to(item, {
            y: itemShift,
            ease: "none",
            scrollTrigger: { trigger: item, start: "top bottom", end: "bottom top", scrub: !0 },
        });
        gsap.to(img, {
            y: imgShift,
            ease: "none",
                ...
                if (triggerElement) {
                    ScrollTrigger.create({
                        ...
                        onEnter: () => {
                            ...
                            if (navCorp) navCorp.classList.add("inverted-c");
                            if (touchbarMobile) {
                                gsap.to(touchbarMobile, { yPercent: 0, duration: 0.4, ease: "power2.out" });
                            }
                        },
                        onLeaveBack: () => {
                            ...
                            if (touchbarMobile) {
                                gsap.to(touchbarMobile, { yPercent: 100, duration: 0.4, ease: "power2.in" });
                            }
                        },
                    });
                }
        ...
        if (texts.length > 0) {
            ...
            function startMainLoop() {
                ...
                texts.forEach((text, index) => {
                    ...
                    loopTl
                        .to({}, { duration: 2 })
                        .to(currentText, { autoAlpha: 0, y: "-100%", duration: 0.5, ease: "power2.inOut" })
                        .fromTo(
                            nextText,
                            { y: "100%" },
                            { autoAlpha: 1, y: "0%", duration: 0.5, ease: "power2.inOut", immediateRender: !1 },
                            "<"
                        );
                });
            }
            const introTl = gsap.timeline({ onComplete: startMainLoop });
            introTl.to(texts[0], { autoAlpha: 1, y: "0%", duration: 0.5, ease: "power2.out" });
        }
    });
    $.validator.addMethod(
        "phoneUS_complete",
    ...
    $("#cta-form, #lp-target-form").validate({
        ...
        unhighlight: function (element) {
            $(element).css("border-bottom", "1px solid var(--white)");
        },
    });
    $("#corp-cta-form").validate({
        ...
        highlight: function (element) {
            $(element).css("background-color", "rgba(200, 15, 15, 0.06)").addClass("error-placeholder");
        },
        unhighlight: function (element) {
            $(element).css("background-color", "").removeClass("error-placeholder");
        },
    });
    ...
    $("#direct-form").validate({
        ...
        highlight: function (element) {
            $(element).css("border", "1px solid var(--error)").addClass("error-placeholder");
        },
        unhighlight: function (element) {
  ...
  function onTurnstileExpired() {
    ...
    if (phoneInputs.length > 0) {
        const phoneMaskOptions = {
            mask: "+1 ({N}00) 000-0000",
            lazy: !1,
            blocks: { N: { mask: IMask.MaskedRange, from: 2, to: 9 }, 0: { mask: IMask.MaskedRange, from: 0, to: 9 } },
        };
    ...
    $("#cta-form, #corp-cta-form, #new-cta-form, #lp-target-form, #direct-form, #price-form").each(function () {
        ...
        if (nativeSuccessMessage.length > 0) {
            ...
            observer.observe(nativeSuccessMessage[0], { attributes: !0 });
        }
    });
